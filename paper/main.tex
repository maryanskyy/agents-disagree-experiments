...
We derive a closed-form threshold $s^*$ (Proposition~\ref{prop:bottleneck}) below which diversity is a liability and above which it becomes an asset.
In a targeted experiment spanning 42 tasks across 7 categories ($N = 210$), a diverse team with judge-based selection achieves a win rate of 0.810 against a single-model baseline, while a homogeneous team scores 0.512---near chance (Hedges' $g = 2.71$ [95\% CI: 2.12, 3.30]).
Most strikingly, judge-based selection outperforms MoA-style synthesis by $\Delta = +0.631$ ($g = 3.86$ [3.14, 4.58])---the synthesis approach loses to the baseline in 82\% of comparisons, winning in zero of 42 tasks.
Exploratory evidence suggests that including a weaker model paradoxically improves performance while reducing cost ($p < 10^{-4}$, not pre-registered).
Practitioners should invest in selector quality before diversifying their generators.
\end{abstract}
...
The formal framework makes this precise. We define a selector quality parameter $s \in [0,1]$ and derive a crossover threshold $s^*$ below which diversity hurts and above which it helps (Proposition~\ref{prop:bottleneck}). The framework further predicts---and the data confirm---that model quality has negligible effect under homogeneity in our data: a homogeneous team of any frontier model performs at chance regardless of selector quality. When all candidates are drawn from the same distribution, there is nothing for even a perfect selector to exploit. Figure~\ref{fig:bottleneck} illustrates the conceptual architecture and crossover geometry.
...
The gap $\Delta(T) = O(T) - M(T)$ measures the team's \textbf{exploitable diversity}: how much a perfect selector could gain over a random one. A \textbf{selector} $\mathcal{S}$ maps the candidate pool to a single output. We parameterize its effectiveness by a scalar $s \in [0, 1]$.

\subsection{The Selection Quality Model}
\label{sec:sqm}

We axiomatize the relationship between team composition, selector quality, and output quality as follows.

\begin{axiom}[Selector-Mediated Quality]
\label{ax:smq}
The expected output quality of team $T$ under a selector of quality $s$ is:
\begin{equation}
...
At $s = 1$, the selector always identifies the best candidate. At $s = 0$, the selector is uninformative---quality collapses to the team mean. Intermediate values interpolate linearly.

This is a modeling choice, not a derived result. We adopt it for three reasons. First, it is exact for $n = 2$ agents under a Gaussian noisy-argmax selector model. Second, it is a controlled first-order approximation for $n > 2$, with approximation error bounded by $\sqrt{\mathrm{Var}(s^*)} \cdot \sigma_\Delta$. Third, its predictions match the observed data: the model's calibrated $s^*$ places the empirical crossover between diversity-helps and diversity-hurts conditions within a narrow confidence band (Section~\ref{sec:calibration}).

We are candid about what this framework is and is not. It is a descriptive model---a lens for organizing empirical findings and generating predictions. Its value lies in predictive power, not mathematical depth. The axiom trades generality for interpretability: a single parameter $s$ compresses the complex behavior of real selectors into a quantity that can be estimated, compared, and communicated to practitioners.

\subsection{The Selection Bottleneck}
\label{sec:bottleneck}

The central result follows directly from the axiom.

\begin{proposition}[The Selection Bottleneck]
\label{prop:bottleneck}
Let $T_h$ be a homogeneous team of the best individual model (expected quality $\mu_\mathrm{best}$) and $T_d$ a diverse team with $M(T_d) < \mu_\mathrm{best}$ and $O(T_d) > O(T_h)$. Then $Q(T_d, s) > Q(T_h, s)$ if and only if $s > s^*$, where:
\begin{equation}
...
\begin{proof}
For the homogeneous team, all candidates are drawn from the same distribution. While $O(T_h) > M(T_h)$ for any non-degenerate distribution, the gap reflects only within-model sampling variance. A selector evaluating outputs from the same model cannot systematically identify the best candidate. We model $O(T_h) \approx M(T_h) \approx \mu_\mathrm{best}$, absorbing the within-model oracle surplus into noise. Thus $Q(T_h, s) = \mu_\mathrm{best}$---constant in $s$.

For the diverse team, $O(T_d) > M(T_d)$ by construction, so $Q(T_d, s)$ is strictly increasing in $s$. Setting $Q(T_d, s^*) = \mu_\mathrm{best}$ and solving:
\begin{align}
s^* \cdot O(T_d) + (1 - s^*) \cdot M(T_d) &= \mu_\mathrm{best} \\
s^* &= \frac{\mu_\mathrm{best} - M(T_d)}{O(T_d) - M(T_d)}.
\end{align}
Below $s^*$, the diverse team's mean dilution dominates its oracle advantage. Above $s^*$, the selector captures enough of the oracle gain to overcome the mean deficit.
\end{proof}
...
\begin{remark}
Proposition~\ref{prop:bottleneck} assumes the linear form of Eq.~\eqref{eq:quality}. The result generalizes to any monotone interpolation $Q(T, s) = g(s) \cdot O(T) + (1 - g(s)) \cdot M(T)$ with $g$ strictly increasing, $g(0) = 0$, $g(1) = 1$; the qualitative structure---a single crossover point---is preserved. The full generalization is provided in the supplementary material.
\end{remark}

\begin{corollary}[Model Quality Irrelevance Under Homogeneity]
\label{cor:irrelevance}
For any homogeneous team $T_h$, $Q(T_h, s) = \mu_\mathrm{model}$ regardless of $s$. A selector has nothing to select when all candidates come from the same distribution.
\end{corollary}

This predicts that two homogeneous teams built from different strong models should perform identically---a strong, falsifiable claim that we test in Section~\ref{sec:confirmatory}.

\begin{corollary}[Synthesis as Degenerate Selection]
\label{cor:synthesis}
A synthesis-based aggregator that blends candidates rather than selecting among them has effective selector quality $s_{\mathrm{synth}} \to 0$, because synthesis does not preserve the identity of individual candidates. For a diverse team, this yields $Q(T_d, 0) = M(T_d) < \mu_{\mathrm{best}}$: diversity becomes a liability because the team mean is diluted by weaker members and no selection mechanism captures the oracle advantage. Synthesis may perform even worse than $M(T_d)$ if the blending process creates ``compromise'' outputs inferior to any individual candidate.
\end{corollary}

This is a central prediction of the framework: MoA-style synthesis should perform poorly with diverse teams not because diversity is harmful, but because synthesis destroys the very signal that diversity creates. We test this prediction directly in Section~\ref{sec:confirmatory}.

\subsection{Optimal Team Size}
\label{sec:teamsize}

The framework also governs how many agents to include. Define the \textbf{marginal contribution} of adding agent $n + 1$:
\begin{equation}
\delta(n, s) = s \cdot \underbrace{[O(n{+}1) - O(n)]}_{\alpha_n \;\geq\; 0} + (1 - s) \cdot \underbrace{[M(n{+}1) - M(n)]}_{\beta_n \;\leq\; 0}.
\label{eq:marginal}
\end{equation}

The first term $\alpha_n$ is the \textbf{oracle gain}; the second $\beta_n$ is the \textbf{mean dilution}. Under standard regularity conditions, the optimal team size $n^*(s)$ is:
\begin{itemize}[nosep]
  \item $n^*(0) = 1$: with a random selector, every additional agent dilutes quality.
  \item $n^*(s)$ is non-decreasing in $s$: better selectors justify larger teams.
  \item $n^*(1) = N$ (pool size): a perfect selector can only benefit from more candidates.
\end{itemize}

Under an equicorrelated Gaussian model with linearly decaying agent quality, the optimal size grows rapidly as $s \to 1$ and collapses to 1 as $s \to 0$. Our empirical data (Section~\ref{sec:exploratory}) are consistent with the prediction of monotonic improvement with diminishing returns under judge-based selection.
...
LLM agents are not independent. In Chatbot Arena~\cite{chiang2024chatbot} (57,477 battles), the tie rate between same-family model pairs is 34.9\%, compared to 28.8\% for cross-family pairs ($\chi^2 = 117.61$, $p < 10^{-27}$; rate difference 6.1 pp [95\% CI: 5.0, 7.2]). In MT-Bench (3,355 human judgments), the same-tier tie rate is 31.9\% versus 20.9\% cross-tier ($\chi^2 = 37.46$, $p < 10^{-6}$; difference 11.0 pp [95\% CI: 7.5, 14.5]). These are not marginal differences---they reflect structural correlations from shared training. Diverse teams reduce $\rho$ by mixing families, providing a second reason why diversity helps under strong selection: not only is the oracle ceiling higher, but the selector's effective accuracy improves because candidates are less redundant. The full CJT generalization is deferred to the supplementary material.

\textbf{Hong and Page.}
Hong and Page~\cite{hong2004groups} proved that diverse groups can outperform ability-selected groups, but their theorem requires an ``effective aggregation mechanism'' without specifying what makes it effective. Our threshold $s^*$ provides the missing quantification: it converts an existence theorem (``conditions exist under which diversity wins'') into a predictive tool (``diversity wins when your selector quality exceeds $s^*$, which you can estimate from pilot data''). The framework also makes explicit three failure regimes: (1)~when $s < s^*$, (2)~when the mean-quality gap is too large, and (3)~when the team is too small for the oracle advantage to accumulate.
...
\textbf{Diversity and selection interact strongly.} Figure~\ref{fig:heatmap} presents BT-corrected win rates across all five cells. The pattern is striking: high win rates appear only when diversity and judge-based selection are combined. A diverse team without a good selector, or a good selector without diversity, both yield near-chance performance.
...
\textbf{Model quality irrelevance.} The regression (Section~\ref{sec:regression}) confirms Corollary~\ref{cor:irrelevance}: the \texttt{diverse\_strong+vote} cell ($\mathrm{BT\text{-}WR} = 0.496$) is statistically indistinguishable from \texttt{homo\_opus+judge} ($\mathrm{BT\text{-}WR} = 0.512$), with $\Delta = -0.016$ [$-0.089$, 0.057], $p = 0.669$. A diverse team without a good selector performs no better than a homogeneous team---voting negates diversity entirely.
...
An important and neglected frontier is not building better individual models---it is building better ways to listen to the choir.
