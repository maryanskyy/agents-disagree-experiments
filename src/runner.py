...
from .utils.token_manager import TokenManager, get_api_base_url, get_api_org_id
...
class ExperimentRunner:
    ...
    def __init__(self, config: RunnerConfig, logger: logging.Logger) -> None:
        ...
        self.cost_tracker = CostTracker(results_dir=config.results_dir)
        ...
        self.topologies = {
            ...
            "self_revision": SelfRevisionTopology(),
        }
    ...
    async def run_manifest(self, manifest: ExperimentManifest) -> dict[str, Any]:
        ...
        summary = {
            "status": "paused" if self._pause_requested else "done",
            "total_runs": len(manifest.runs),
    ...
    async def _execute_one(self, run_spec: RunSpec, *, state: dict[str, int], total: int, start_time: float) -> None:
        ...
        async with self._state_lock:
            ...
            if done % self.config.progress_every == 0 or done == total:
                ...
                self.logger.info(
                    "Progress: %d/%d complete | failed=%d | eta=%.1fs | tokens=%d",
                    done,
                    total,
                    state["failed"],
                    progress.eta_seconds or 0.0,
                    self.cost_tracker.total_input_tokens + self.cost_tracker.total_output_tokens,
                )
    ...
    def _build_progress_snapshot(self, *, done: int, total: int, failed: int, start_time: float) -> ProgressSnapshot:
        ...
        return ProgressSnapshot(
            ...
            failed_runs=failed,
            eta_seconds=eta,
            status="paused" if self._pause_requested else "running",
            warning=self._pause_reason,
        )

    def _check_cost_guardrail(self) -> None:
        """Token budget guardrail (placeholder — cost tracking removed)."""
        pass  # Cost guardrail removed; add token-based limits if needed
    ...
    _THROTTLED_MODELS: dict[str, int] = {"gpt-5.4": 3, "gpt-5.2": 3, "gpt-5.3-chat-latest": 3}

    def _get_model_semaphore(self, model_alias: str) -> asyncio.Semaphore:
        if model_alias not in self._model_semaphores:
            limit = self._THROTTLED_MODELS.get(model_alias, 100)
            self._model_semaphores[model_alias] = asyncio.Semaphore(limit)
        return self._model_semaphores[model_alias]
    ...
    async def _invoke_agent_inner(
        ...
        for _retry in range(_EMPTY_RETRIES):
            if response.text and response.text.strip():
                break
            backoff = min(2 ** _retry, 30) + random.uniform(0, 2)
            self.logger.warning(
                "Empty output from %s on attempt %d/%d, run=%s (backoff %.1fs)",
                model_alias, _retry + 1, _EMPTY_RETRIES, run_spec.id, backoff,
            )
    ...
    async def _execute_run(self, run_spec: RunSpec) -> dict[str, Any]:
        ...
        async def invoke_agent(
            ...
            sem = self._get_model_semaphore(model_alias)
            async with sem:
                return await self._invoke_agent_inner(
                    run_spec, agent_idx, model_alias, system_prompt,
                    user_prompt, temperature, context,
                )
    ...
    async def _evaluate_final_quality(
        ...
        *,
        ...
        # NOTE: Skip shortcut for hierarchical topology -- root panel only compares
        # cluster winners, not all leaf outputs, producing broken metrics.
        can_reuse = (
            run_spec.consensus == "judge_based"
            and run_spec.topology not in ("hierarchical",)
        )
        panel_payload = consensus_metadata.get("judge_panel") if can_reuse else None
    ...
    async def _build_consensus(self, run_spec: RunSpec, *, judge_panel: JudgePanel):
        ...
        if run_spec.consensus == "synthesis":
            synth_model = run_spec.metadata.get("synthesizer_model", "claude-sonnet-4-6")
            synth_client = self._get_client(synth_model)

            def _cost_recorder(model_name, response, tag, *, _block=run_spec.block_id, _run=run_spec.id):
                self.cost_tracker.record_call(
                    block_id=_block, run_id=_run, model_name=model_name,
                    input_tokens=response.input_tokens, output_tokens=response.output_tokens,
                    latency_ms=response.latency_ms, tag=tag,
                )

            return SynthesisConsensus(
                synthesizer_client=synth_client,
                model_name=synth_model,
                cost_recorder=_cost_recorder,
            )
    ...
    def _create_client(self, *, provider: str, model_alias: str, api_model: str, token: str) -> BaseModelClient:
        ...
        api_base_url = f"{api_base}/v1" if api_base else None
        extra_hdrs = {"OpenAI-Organization": org_id} if org_id else {}
